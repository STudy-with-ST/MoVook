...
public class JwtServiceImpl implements JwtService {
    ...
    private static final int REFRESH_TOKEN_EXPIRE_HOURS = 30; // 주단위

    @Override
    public <T> String createAccessToken(String key, T data) {

		return create(key, data, "access-token", 1000 * 60 * ACCESS_TOKEN_EXPIRE_MINUTES); // 단위가 ms임
//        return create(key, data, "access-token", 1000 * 10 * ACCESS_TOKEN_EXPIRE_MINUTES);
    ...
    public <T> String createRefreshToken(String key, T data) {
        //
		return create(key, data, "refresh-token", 1000 * 60 * REFRESH_TOKEN_EXPIRE_HOURS);
